...
from sklearn.metrics import precision_recall_fscore_support, accuracy_score, average_precision_score, f1_score
...
def predict(dataset, label2id, id2label, model_name, out_dir, num_epochs):
    ...
    train_dataset = dataset.map(partial(tokenize_and_align_labels, tag_type="tags"), batched=True, )
...
dataset_tokenized = dataset.map(partial(tokenize_and_align_labels, tag_type="tags_gold"), batched=True, )
